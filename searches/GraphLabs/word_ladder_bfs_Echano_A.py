...
def generate_path(current, explored):# generates path from explored
   list = [current]
   count = 0
...
def main():
    ...
    #   makes sure no invalids and words are actually in the dictionary
    if (len(testWord) == 6 and len(goalState) == 6 and testWord in dict.keys() and goalState in dict.keys()):

        shortestPath = BFsearch(testWord, goalState, dict)  #   actually finds path using BFS
        ...
        print("Not valid input(s)/No solution") #No solution
