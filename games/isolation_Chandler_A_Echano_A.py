...
# Name: Alexis Echano and Arwen Chandler   Date: 12/7/2018
...
class OpenMoveEvalFn():
    ...
    def score(self, game, maximizing_player_turn=True):
        ...
        temp = len(game.get_legal_moves_of_queen())
        if game.get_active_queen() == 'queen1':
            return len(game.get_opponent_moves()[1]) - temp
        else:
            return temp - len(game.get_opponent_moves()[2])
...
class CustomPlayer():
    ...
    def minimax(self, game, time_left, depth=float("inf"), maximizing_player=True):
        ...
        if depth == 0 or len(moves) == 0:  #base case
            ...
            for l in moves:
                temp = game.forecast_move(l, game.get_active_players_queen())
                move, queen, value = self.minimax(temp, depth - 1, not maximizing_player)

                test = get_action(value, best_val)
    ...
    def alphabeta(self, game, time_left, depth=float("inf"), alpha=float("-inf"), beta=float("inf"), maximizing_player=True):
        # TODO: finish this function! BUT NOT REALLY
        return None, None, 0
...
class Board:
    ...
    def __init__(self, player_1, player_2, width=5, height=5):   #change box size here
        self.width=width
        self.height=height
    ...
    def get_opponent_moves(self):
        #changed so that you get access to even the inactive players queens.
...
if __name__ == '__main__':
    ...
    player_1.__name__ = "Alexis"
    player_2.__name__ = "Arwen"
